...
describe('Game', () => {
  ...
  beforeEach(() => {
	...
	repo.get_texture.mockResolvedValue({'name': 'petscii-sky', 
  ...
  it('wraps round to zero, for now, when you go over the max engagement level', async () => {
  	game.engagement = game.max_engagement;
  	await game.inc_engagement();
  	expect(game.engagement).toEqual(0);
  });
